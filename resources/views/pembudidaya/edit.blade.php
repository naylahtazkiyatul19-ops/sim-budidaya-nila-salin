...
<head>
    ...
    <title>Edit Pembudidaya - SIM Nila Salin</title>
...
<body>
    ...
    <div class="main">
        <div class="page-title">
            <h1><i class="fas fa-user-edit text-cyan"></i> Edit <span class="highlight">Pembudidaya</span></h1>
            <p>Edit data pembudidaya: <strong>{{ $pembudidaya->nama }}</strong></p>
        </div>

        <div class="card">
            <form action="{{ route('pembudidaya.update', $pembudidaya->id) }}" method="POST">
                @csrf @method('PUT')

                <div class="mb-3">
                    <label class="form-label"><i class="fas fa-user text-cyan"></i> Nama Lengkap <span class="text-danger">*</span></label>
                    <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror" value="{{ old('nama', $pembudidaya->nama) }}" required>
                    @error('nama') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="mb-3">
                    <label class="form-label"><i class="fas fa-map-marker-alt text-cyan"></i> Alamat</label>
                    <textarea name="alamat" class="form-control @error('alamat') is-invalid @enderror" rows="2">{{ old('alamat', $pembudidaya->alamat) }}</textarea>
                    @error('alamat') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="mb-3">
                    <label class="form-label"><i class="fas fa-phone text-cyan"></i> Nomor HP</label>
                    <input type="text" name="no_hp" class="form-control @error('no_hp') is-invalid @enderror" value="{{ old('no_hp', $pembudidaya->no_hp) }}">
                    @error('no_hp') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>

                <div class="mb-3">
                    <label class="form-label"><i class="fas fa-store text-cyan"></i> Nama UMKM</label>
                    <input type="text" name="nama_umkm" class="form-control @error('nama_umkm') is-invalid @enderror" value="{{ old('nama_umkm', $pembudidaya->nama_umkm) }}">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror" rows="2">{{ old('deskripsi', $pembudidaya->deskripsi) }}</textarea>
